	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="适宜人群" prop="shiyirenqun" >
					<el-select :disabled="ro.shiyirenqun" v-model="ruleForm.shiyirenqun" placeholder="请选择适宜人群" >
						<el-option
							v-for="(item,index) in shiyirenqunOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="适宜人群" prop="shiyirenqun" >
					<el-input v-model="ruleForm.shiyirenqun"
						placeholder="适宜人群" readonly></el-input>
				</el-form-item>
				...
				<el-form-item v-else class="input" label="功效" prop="gongxiao" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="搭配禁忌" prop="dapeijinji" >
					<el-input v-model="ruleForm.dapeijinji" placeholder="搭配禁忌" clearable  :readonly="ro.dapeijinji"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="搭配禁忌" prop="dapeijinji" >
					<el-input v-model="ruleForm.dapeijinji" placeholder="搭配禁忌" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="原材料" prop="yuancailiao" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.yuancailiao" label="原材料" prop="yuancailiao" >
				<span class="text">{{ruleForm.yuancailiao}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="制作方法" prop="zhizuofangfa" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.zhizuofangfa" 
					class="editor"
					myQuillEditor="zhizuofangfa"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.zhizuofangfa" label="制作方法" prop="zhizuofangfa" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.zhizuofangfa"></span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.$http({
					url: `option/shiyirenqun/shiyirenqun`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.shiyirenqunOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.zhizuofangfa = this.ruleForm.zhizuofangfa.replace(reg,'../../../springboot6djh2h8f/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiankangshipu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiankangshipuCrossAddOrUpdateFlag = false;
											this.parent.search();
