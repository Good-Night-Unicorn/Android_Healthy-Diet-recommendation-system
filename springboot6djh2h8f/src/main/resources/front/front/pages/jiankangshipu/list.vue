			...
			<view :style='{"minHeight":"100vh","width":"100%","padding":"0 20rpx 128rpx","position":"relative","background":"#fff3e8","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 0px)","padding":"20rpx 0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"0px solid #174add","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"8rpx","background":"#ffffff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shipumingcheng" type="text" placeholder="食谱名称" ></input>
					</view>
				...
				<view :style='{"alignContent":"flex-start","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					<scroll-view scroll-x="true" class="category-one" :style='{"padding":"24rpx 24rpx 0","borderColor":"#fed2a8","whiteSpace":"nowrap","overflow":"hidden","borderRadius":"20rpx 20rpx 0 0","background":"#fff","borderWidth":"2rpx 2rpx 0","width":"100%","borderStyle":"solid"}'>
						<view :class='categoryName === item.shiyirenqun ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.shiyirenqun)">{{item.shiyirenqun}}</view>
					</scroll-view>
					<!-- 样式2 -->
					<view class="list" :style='{"width":"100%","padding":"40rpx 20rpx 20rpx","margin":"0px 0 0","background":"#ffe6cd","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"alignContent":"center","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"padding":"0","margin":"0","color":"#888","flexWrap":"wrap","display":"flex","width":"calc(100% - 200rpx)","fontSize":"28rpx","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.shipumingcheng}}</view>
								...
								<view :style='{"padding":"0 20rpx","margin":"0 0px 0 0","color":"#fff","alignItems":"center","background":"#fa861c","display":"flex","order":"9"}'>
								...
								<view :style='{"padding":"0 20rpx","margin":"0 0px 0 0","color":"#fff","alignItems":"center","background":"#fa861c","display":"flex","order":"8"}'>
								...
								<view :style='{"padding":"0 20rpx","color":"#fff","alignItems":"center","background":"#fa861c","display":"flex","order":"10"}'>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #fa861c","padding":"10rpx","borderRadius":"8rpx","background":"#fff3e8","display":"flex"}' v-if="(userid && isAuth('jiankangshipu','修改')) || (!userid && isAuthFront('jiankangshipu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","borderRadius":"8rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('jiankangshipu','删除')) || (!userid && isAuthFront('jiankangshipu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #fa861c","padding":"10rpx","borderRadius":"8rpx","background":"#fff3e8","display":"flex"}' v-if="(userid && isAuth('jiankangshipu','修改')) || (!userid && isAuthFront('jiankangshipu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fa861c","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fa861c","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","borderRadius":"8rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('jiankangshipu','删除')) || (!userid && isAuthFront('jiankangshipu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","right":"20rpx","outline":"none","borderRadius":"100%","top":"144rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('jiankangshipu','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","right":"20rpx","outline":"none","borderRadius":"100%","top":"144rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('jiankangshipu','新增')" class="add-btn" @click="onAddTap()">新增</button>
			</view>
		</view>
	</mescroll-uni>
</template>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('shiyirenqun', {page:1,limit:100});
            } else {
                res = await this.$api.list('shiyirenqun', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,shiyirenqun:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.shiyirenqun = '%' + this.categoryName + '%'
				}
				if(this.searchForm.shipumingcheng){
					params['shipumingcheng'] = '%' + this.searchForm.shipumingcheng + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiankangshipu`, params);
                } else {
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiankangshipu', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.shipumingcheng){
					searchForm['shipumingcheng'] = '%' + this.searchForm.shipumingcheng + '%'
				}
				if(this.categoryName!='全部'){
					searchForm.shiyirenqun = '%' + this.categoryName + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiankangshipu`, searchForm);
                } else {
                    res = await this.$api.list(`jiankangshipu`, searchForm);
                }
				// 如果是第一页数据置空
