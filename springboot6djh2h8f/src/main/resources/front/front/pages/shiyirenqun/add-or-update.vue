	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#fff3e8","height":"auto"}'>
		<form :style='{"width":"100%","padding":"48rpx 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"4rpx 40rpx","margin":"0 0 40rpx 0","borderColor":"#dfdfdf","alignItems":"center","borderRadius":"0px","borderWidth":"0px 0 2rpx 0","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"30rpx","height":"80rpx"}' :disabled="ro.shiyirenqun" v-model="ruleForm.shiyirenqun" placeholder="适宜人群"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`shiyirenqun`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				//跨表计算判断
				var obj;
				if((!this.ruleForm.shiyirenqun)){
					this.$utils.msg(`适宜人群不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`shiyirenqun`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`shiyirenqun`, this.ruleForm);
						}else{
							oet = await this.$api.add(`shiyirenqun`, this.ruleForm);
						}
						if(this.cross){
