	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#fff3e8","height":"auto"}'>
		<form :style='{"width":"100%","padding":"48rpx 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"10rpx 20rpx","margin":"0","borderColor":"#1ccb96","borderRadius":"20rpx","borderWidth":"0px 0","background":"none","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"100%","lineHeight":"80rpx","fontSize":"30rpx","color":"#666","fontWeight":"500"}' class="title">评论内容</view>
				<xia-editor ref="content" :style='{"minHeight":"300rpx","border":"2rpx solid #dfdfdf","padding":"20rpx","borderRadius":"20rpx","width":"100%","fontSize":"30rpx","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjiankangshipu`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjiankangshipu`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussjiankangshipu`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussjiankangshipu`, this.ruleForm);
						}
						uni.setStorageSync('discussjiankangshipuCleanType',true);
						if(this.cross){
