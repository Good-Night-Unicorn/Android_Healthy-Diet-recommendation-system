			...
			<view :style='{"minHeight":"100vh","width":"100%","padding":"0 20rpx 128rpx","position":"relative","background":"#fff3e8","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 0px)","padding":"20rpx 0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"0px solid #174add","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"8rpx","background":"#ffffff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.jilubiaoti" type="text" placeholder="记录标题" ></input>
					</view>
				...
				<view :style='{"alignContent":"flex-start","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					...
					<view class="list" :style='{"width":"100%","padding":"40rpx 20rpx 20rpx","margin":"0px 0 0","background":"#ffe6cd","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"alignContent":"center","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' v-for="(product,index) in list" :key="index">
							<image :style='{"width":"180rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"12rpx","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.tupian)" :src="product.tupian.split(',')[0]"></image>
							<image :style='{"width":"180rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"12rpx","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.tupian?baseUrl+product.tupian.split(',')[0]:''"></image>
							...
							<view class="listmr" :style='{"padding":"0","margin":"0","color":"#888","flexWrap":"wrap","display":"flex","width":"calc(100% - 200rpx)","fontSize":"28rpx","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.jilubiaoti}}</view>
								...
								<view :style='{"width":"auto","padding":"0 38rpx 0 28rpx","alignItems":"center","background":"#fff","display":"flex"}'>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #fa861c","padding":"10rpx","borderRadius":"8rpx","background":"#fff3e8","display":"flex"}' v-if="(userid && isAuth('yinshijilu','修改')) || (!userid && isAuthFront('yinshijilu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","borderRadius":"8rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('yinshijilu','删除')) || (!userid && isAuthFront('yinshijilu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #fa861c","padding":"10rpx","borderRadius":"8rpx","background":"#fff3e8","display":"flex"}' v-if="(userid && isAuth('yinshijilu','修改')) || (!userid && isAuthFront('yinshijilu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fa861c","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fa861c","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","borderRadius":"8rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('yinshijilu','删除')) || (!userid && isAuthFront('yinshijilu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #00000030","color":"#fff","outline":"none","borderRadius":"100%","top":"144rpx","left":"10rpx","background":"#ffaa33","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' class="add-btn" @click="screenBoxShow=true">筛</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","right":"20rpx","outline":"none","borderRadius":"100%","top":"144rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('yinshijilu','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","right":"20rpx","outline":"none","borderRadius":"100%","top":"144rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('yinshijilu','新增')" class="add-btn" @click="onAddTap()">新增</button>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>记录类型</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="jiluleixingChange(item)" v-for="(item,index) in jiluleixingOptions" :key="index" class="screenTab" :class="searchForm.jiluleixing==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
					<view :style='{"width":"100%","padding":"40rpx 0 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<div :style='{"border":"2rpx solid #ddd","color":"#666","borderRadius":"8rpx","textAlign":"center","background":"#ededed","width":"30%","lineHeight":"68rpx","height":"68rpx"}' @click="screenReset">重置</div>
						<div :style='{"color":"#fff","borderRadius":"8rpx","textAlign":"center","background":"#fa861c","width":"30%","lineHeight":"68rpx","height":"68rpx"}' @click="search">搜索</div>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.jilubiaoti){
					params['jilubiaoti'] = '%' + this.searchForm.jilubiaoti + '%'
				}
				if(this.searchForm.jiluleixing){
					params['jiluleixing'] = '%' + this.searchForm.jiluleixing + '%'
				}
			...
			async search(){
				...
				if(this.searchForm.jilubiaoti){
					searchForm['jilubiaoti'] = '%' + this.searchForm.jilubiaoti + '%'
				}
				if(this.searchForm.jiluleixing){
					searchForm['jiluleixing'] = '%' + this.searchForm.jiluleixing + '%'
				}
                let res = {};
                if(this.userid) {
